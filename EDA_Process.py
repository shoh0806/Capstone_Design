...
print("--- 데이터 로딩 중 ---")
movies = pd.scan_csv("movies (1).csv")
ratings = pd.scan_csv("ratings (1).csv")
# zip 컬럼을 문자열로 강제 지정하여 에러 방지
users = pd.scan_csv("users (1).csv", schema_overrides={"zip": pd.String})
...
user_activity = df_full.group_by("userId").agg(
    pd.count("rating").alias("rating_count")
...
if "gender" in df_full.columns and "age" in df_full.columns:
    ...
    demo_stats = df_full.group_by(["gender", "age"]).agg(
        pd.col("rating").mean().alias("avg_rating")
...
movie_stats = df_full.group_by("title").agg([
    pd.col("rating").count().alias("count"),
    pd.col("rating").mean().alias("mean")
]).filter(pd.col("count") > 500).to_pandas() # Pandas로 변환
...
if "timestamp" in df_full.columns:
    ...
    df_time = df_full.with_columns(
        pd.from_epoch("timestamp", time_unit="s").dt.year().alias("year")
    )
    yearly_avg = df_time.group_by("year").agg(pd.col("rating").mean()).sort("year").to_pandas()
...
>>>>>>> 96c6b36cecb13810ee0d5ecc651bba864c022297
print("\n--- 모든 EDA 프로세스 완료 ---")